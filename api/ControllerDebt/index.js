...
export const management = async (app) => {
    app.get(prefixApi, helper.authenToken, async (req, res) => {
        ...
        if (validator.isDefine(req.query.key)) {
            query_key = {
                ...query_key,
                $or:[{"user.user_fullname":{$regex:".*"+req.query.key+".*", $options:"$i"}},{"user.user_phone":{$regex:".*"+req.query.key+".*"}}]
            }
        }
        if (validator.ObjectId.isValid(req.query.key)) {
            query_key = {
                $or:[{_id:validator.ObjectId(req.query.key)},{id_form:validator.ObjectId(req.query.key)}]
            }
        }
        let query_total = {}
...
export const getDetailReport = async (app) => {
    app.get(prefixApi +"/report", helper.authenToken, async (req, res) => {
        ...
        const currentDebt = await ModelDebt.find(query)
