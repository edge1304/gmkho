        ...
        try
        {
            ...
            for (let i = 0; i < arrProduct.length; i++){
                await ModelProduct.findByIdAndUpdate(arrProduct[i].id_product,{
                    ...
                    $push:{
                        product_note:`${ validator.dateTimeZone().currentTime} xuất hàng từ đơn hàng trên web.Mã phiếu xuất ${insertFormExport._id} từ kho ${data_warehouse.warehouse_name}, nv = ${id_employee}, kh = ${dataUser._id} - ${dataUser.user_fullname}`
                    }
                })
            }
            ...
            await new ModelNotificationUser({
                ...
                notification_content:`Đơn hàng của bạn đã được giao. Mã đơn hàng của bạn là ${dataOrder._id}`,
            ...
            validator.notifyTopic(dataUser.user_phone,"Thông báo",`Đơn hàng của bạn đã được giao. Mã đơn hàng của bạn là ${dataOrder._id}`)
        ...
        try
        {
           ...
           const arrOrderProduct = []
            ...
            const insertOrder = await new ModelOrder({
                ...
                order_note:"nhân viên tạo từ web"
            }).save()
