        ...
        callAPI('GET',API_PRODUCT,{
            ...
            for (let i = 0; i < dataInternal.interal_order_product.length; i++){
                if (dataInternal.interal_order_product[i].id_subcategory.toString() == data.id_subcategory.toString()) {
                    ...
                    $($(tr).find('input')[1]).val(money(dataInternal.interal_order_product[i].subcategory_export_price))
                    $($(tr).find('input')[2]).val(money(dataInternal.interal_order_product[i].subcategory_vat))
                    $($(tr).find('input')[3]).val(money(dataInternal.interal_order_product[i].subcategory_ck))
                    $($(tr).find('input')[4]).val(money(dataInternal.interal_order_product[i].subcategory_discount))
                    $($(tr).find('input')[5]).val(money(dataInternal.interal_order_product[i].product_warranty))
                    break
