...
{
    drawTable()
    callAPI('GET', `${API_ORDER}/checkPermission/export?`, { id_order: id_order }, (data) => {
        dataOrder = data.dataOrder
        data.fundbooks.map(fund =>{
            $("#selectTypePayment").append(`<option value="${fund._id}">${fund.fundbook_name}</option>`)
        })
        $("#div_find_supplier input").val(dataOrder.user_fullname)
        $($(".header-table input")[1]).val(dataOrder.user_phone)
        $($(".header-table input")[2]).val(dataOrder.user_address)
        $($(".header-table input")[3]).val(dataOrder.user_point)

        money_point = dataOrder.money_point
        money_voucher_code = dataOrder.money_voucher_code
        $("#input_code_discount").val(dataOrder.voucher_code)
        $("#input_point").val(dataOrder.point_number)
...
$("#btnConfirm").click(e => {
    ...
    const code_discount = is_click_voucher ? $("#input_code_discount").val().trim() : null
